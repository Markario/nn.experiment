...
public class GenericWeightExperiment<T extends GenericWeight<T>> {
    ...
    public static void main(String[] args) {
        ...
        for (int gen = 0; gen < 50000; gen++) {
            ...
            if (experiment.getFitness(0) >= 622) {
                gen = 50000;
            }
        ...
        while (input < Math.PI * 2) {
            ...
            double sum = outputs.stream().mapToDouble(value -> value.value).sum();
            Trace.v(String.format("%f, %f, %f", input, correctValue, sum));
        ...
        Trace.v("population: "+new GsonBuilder().create().toJson(experiment.genetics.getPopulation()));
    }
